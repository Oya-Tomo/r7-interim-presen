...
#let bg_section = section_box("背景", 100%, 9%)[
  ...
  しかし，不整地走行のための強化学習モデルは学習済みのモデルウェイトが公開されておらず，ほとんどの場合，独自に訓練する必要がある．
...
#let comp = section_box("比較・考察", 100%, 21%)[
  Table 1 に示す3つの手法を実装し，走行性能の比較を行った．
  ...
  #figure(
    ...
    caption: "平地・不整地走行の可否検証結果",
  )

  　MLPでは過去の情報を用いず，RNNでは効率的な過去情報の伝搬を学習することができなかったため，不整地走行は困難であった．
  一方でSLRは，過去の観測情報を時系列的に一貫性のある潜在表現に変換して，効率的に利用することで路面状況を推測することができ，不整地走行が可能になったと考えられる．
]
...
#let slr_section = section_box("先行研究: Self-learning Latent Representation", 100%, 38.5%)[
  　Self-learning Latent Representation (以下，SLR) はPPOをベースとした手法であり，過去の観測情報を埋め込んでMLPに入力することで，不整地走行に必要な情報を効率的に学習することを目的としている．
...
#let plan_section = section_box("今後の展望", 100%, 13%)[
  　現状の手法では，RGBカメラや深度情報などの視覚情報を利用していないため，障害物の認識が困難であり複雑な不整地走行には対応できていない．\
